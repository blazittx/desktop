...
const createParticlePool = (size: number): IParticle[] => {
...
// Get a random number between 0 and 1 using crypto
...
const BackgroundAnimation: React.FC<IBackgroundAnimationProps> = ({
  ...
  const globalHueSeedRef = useRef(getRandomNumber() * 360)
  const frameIdRef = useRef<number | null>(null)
  const noiseRef = useRef<any>(null)
  const circlePathRef = useRef<Path2D | null>(null)
  const lastFrameTimeRef = useRef(0)
  const visibleParticlesRef = useRef<IParticle[]>([])
  const particlePoolRef = useRef<IParticle[]>([])
  ...
  useEffect(() => {
    ...
    noiseRef.current = (function () {
      ...
      module.simplex3 = function (xin: number, yin: number, zin: number) {
    ...
    noiseRef.current.seed(getRandomNumber())
    ...
    for (let x = 0; x < canvas.width; x += CONFIG.GRID_STEP) {
      for (let y = 0; y < canvas.height; y += CONFIG.GRID_STEP) {
        if (particleIndex >= particlePoolRef.current.length) {
          break
        }
    ...
    const updateVisibleParticles = (t: number, t2: number) => {
    ...
    function animate() {
      ...
      for (
        let i = 0;
        i < visibleParticlesRef.current.length;
        i += CONFIG.RENDER.BATCH_SIZE
      ) {
        const end = Math.min(
          i + CONFIG.RENDER.BATCH_SIZE,
          visibleParticlesRef.current.length
        )
        ...
        for (let j = i; j < end; j++) {
          ...
          const baseHue =
            seed +
            noiseRef.current.simplex3(pos.nx2, pos.ny2, t) *
              CONFIG.COLOR.HUE_VARIATION
          const hueVariation =
            CONFIG.COLOR.HUE_OFFSET +
            noiseRef.current.simplex3(pos.nx3, pos.ny3, t) *
              CONFIG.COLOR.HUE_VARIATION
